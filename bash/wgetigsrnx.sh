...
## discription           : downloads igs rinex files.
## uses                  : wget
## notes                 : this is a newer version of the script downloadigs.sh
##                         Updated lists of IGS stations can be found at:
##                         ftp://igscb.jpl.nasa.gov/pub/station
## TODO                  :
## detailed update list  : DEC-2013 corrected exit codes
##                       : OCT-2014
                           LAST_UPDATE=OCT-2014
##
################################################################################
...
function help {
  echo "/******************************************************************************************/"
  echo " Program Name : wgetigsrnx.sh"
  echo " Version      : $VERSION"
  echo " Last Update  : $LAST_UPDATE"
  echo ""
  echo " Purpose : Download IGS stations rinex files for a specific date. If the file already exists,"
  echo "           (in capital letters, compressed or not, Hatanaka or RINEX) then it will not "
  echo "           be downloaded, except if the -r switch is given. In case a file already exists and"
  echo "           does not to be downloaded, this file is counted as a downloaded one in the integer returned."
  echo ""
...
IGSURL1=ftp://cddis.gsfc.nasa.gov/pub/gps/data/daily ## url for igs rinex files
IGSURL2=ftp://igs.bkg.bund.de/IGS/obs                ## url for igs rinex files
...
for station in ${STARRAY[*]}; do
  ...
  if [ "$STACOUNTER" -lt "$MAXST" ]; then
    ...
    if [ ${PROCED_TO_DOWNLOAD} == "NO" ]; then ## do not proceed to download; file available
      ...
      # try downloading from cddis
      wget -nc --tries="$NUMOFTRIES" -q -O ${OUTDIR}/${station}${DOY}0.${YR2}d.Z \
        ${IGSURL1}/${YEAR}/${DOY}/${YR2}d/${station}${DOY}0.${YR2}d.Z
      # if station not downloaded, try BKG url
      if [ "$?" -ne 0 ]; then
        if [ $QUIET == "NO" ]; then echo "## Station $station not found in cddis; trying in bkg ..."; fi
        rm -f ${OUTDIR}/${station}${DOY}0.${YR2}d.Z 2>/dev/null ## remove empty file if any
        wget -nc --tries="$NUMOFTRIES" -q -O ${OUTDIR}/${station}${DOY}0.${YR2}d.Z \
          ${IGSURL2}/${YEAR}/${DOY}/${station}${DOY}0.${YR2}d.Z
        # check if it is downloaded from bkg
        if [ "$?" -ne 0 ]; then

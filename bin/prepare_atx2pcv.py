...
if not os.path.isfile( args.sta_file ):
    print >> sys.stderr, 'ERROR. Invalid sta file (%s)'%args.sta_file
    sys.exit(1)
sta_filename   = os.path.basename( args.sta_file )
sta_src_dir    = os.path.dirname( args.sta_file )
symlink_source = args.sta_file
symlink_target = os.path.join(campaign_dir, 'OUT', sta_filename)
if symlink_source != symlink_target:
    ## if such file already exists, first move it
    if os.path.isfile( symlink_target ):
        shutil.move( symlink_target, symlink_target + '.bck' )
...
pcf_inst.set_variable(var_name='STAINF', val=sta_filename)
...
doy           = 1
session       = 0
sys_command_2 = '%s %04i %03i%01i %s'%(perl_script, year, doy, sesssion, args.campaign)

if args.shell_script is not None:
    if year >= 2000 : yr2 = year - 2000
    else            : yr2 = year - 1900
    bpe_dir = os.path.join(campaign_dir, 'BPE')
    log_proc= os.path.join(campaign_dir, 'AP%02i%03i%01i_001_000.LOG'%(yr2, doy))

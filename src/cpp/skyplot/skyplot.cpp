...
const double scaleBar = 0.04;    // was 0.04 for full page
...
int main( )
{
  ...
  double currMJD, currHour, epochNum = 0.0;
  double dx,dy,az;
  ...
  ofstream out("cf2sky.log");
  if ( !out )
    {
      cerr << "Error opening cf2sky.log ! " << endl;
      return -1;
    }
  out << endl << "Program CF2SKY   Version 17 December 2003" << endl << endl;
  out.setf( ios::fixed, ios::floatfield );

  ifstream inp("cf2sky.inp");
  if ( !inp )
    {
      cerr << "Error opening cf2sky.inp ! " << endl;
      out << endl << "Error opening cf2sky.inp ! " << endl << endl;
      return -1;
    }
...
std::cout<<"\ngot line: ["<<stemp<<"]";
//  inp >> xsta[0] >> xsta[1] >> xsta[2] ;   // Get XYZ from RINEX OBS instead
  inp >> cutoffAngle;
  inp >> rinexObsFile;
  inp >> teqcPltFile;
  inp.close();
// Print options to stdout
//std::cout << "\nOptions read from cf2sky.inp";
//std::cout << "\n-------------------------------------------------------";
//std::cout << "\nStart Date    : "<<year1 <<" "<< mon1 << " " << day1 << " " << hr1 << " " << min1 << " " << sec1;
//std::cout << "\nStop Date     : "<<year2 <<" "<< mon2 << " " << day2 << " " << hr2 << " " << min2 << " " << sec2;
//std::cout << "\nOrbit file    : "<< orbfile;
//std::cout << "\nCut Off Angle : "<< cutoffAngle;
//std::cout << "\nRinex file    : "<< rinexObsFile;
//std::cout << "\nTeqc Plot File: "<< teqcPltFile;

// Open the RINEX OBS file to get the Approx. X,Y,Z coordinates
  ifstream obs(rinexObsFile.c_str());
  if ( !obs )
  {
    cerr << "Error opening " << rinexObsFile << endl;
    return -1;
  }
...
//  out << xsta[0] << " " << xsta[1] << " " << xsta[2] << " " << endl;
  ...
  out << rinexObsFile << endl;
  out << teqcPltFile << endl;
...
//         out << "\n\nuser supplied XYZ, rather than LLH" << endl;
        ...
        htt = xsta[2];
//         out << "\n\nuser supplied LLH, rather than XYZ " << endl;
         LLHsupplied = true;
  }
  ...
  outtt << "0 2.1  10 0 0 CM Lat: "
  #endif
  #ifdef GMT5
  outtt << "0 2.2 14p,Helvetica,black " << stemp << endl;
  outtt << "0 2.1 10p,Helvetica,black  Lat: "
  #endif
   << setw(10) << setprecision(4) << lat * 180.0 /jpi << "\260    Lon:  "
   << setw(10) << setprecision(4) << lon * 180.0 /jpi << "\260    Ell Ht:  "
   << setw(10) << setprecision(1) << htt <<  " (m) " <<
   endl;
  #ifdef GMT4
  outtt << "0 2.0  10 0 0 CM GPS Time:   Start "
  #endif
  #ifdef GMT5
  outtt << "0 2.0 10p,Helvetica,black GPS Time:   Start "
  #endif
  << setw(2) << setprecision(0)
  << year1 << "/" << padZeros(mon1) << "/"  <<
  padZeros(day1) << "  " << padZeros(hr1) << ":"
  << padZeros(min1) << ":" << padZeros(sec1) << "      Stop "
  ...
  #ifdef GMT5
  outbat << "echo off " << endl;                                                         // -X0.75 -Y1.0
  outbat << "psxy elevRings.dat  -R-1.6/1.6/-1.6/1.6 -JX18.0  -W1.0p,black,solid -G230 -V -K -P -X1.5 -Y2.0 > skyplot.ps " << endl;
  outbat << "psxy cutoffRing.dat -R -JX -W.7p,black,-  -G255 -V  -O -K -P >> skyplot.ps " << endl;
  outbat << "psxy elevRings.dat  -R -JX -W1.0p,black,solid -V  -O -K -P >> skyplot.ps " << endl;
  outbat << "#psxy elevRings.dat  -R -JX -W0.5p,red,solid  -V  -O -K -P >> skyplot.ps " << endl;
  outbat << "pstext title.txt -R -JX -F+a0+jCM+f -N -V  -O -K -P >> skyplot.ps " << endl;
  outbat << "psvelo mp.xy  -R -JX  -L -Ggreen -W1.5p,green,solid -Se1.5/0.95/0 -A0.0 -N  -h0 -O -K -P -V >>  skyplot.ps " << endl;
  #endif
  ...
  system(" rm *.sat.xy arrows.xy elevRings.dat ring.txt nesw.txt skyplot.ps .gmt* 2> rm.me");
  ofstream satcoords;
  ...
  bool ignore_line = false;
   ...
   if( !cfplt )
   {
     ...
     << "Now terminating Program CF2PS." << endl << endl;
     cerr << "Error! Cannot open file: " << teqcPltFile.c_str() << endl
     << "Now terminating Program CF2SKY." << endl << endl;
     return -1;
   }
   ...
   if (!ignore_line) getline( cfplt, buf );  // **** read the third line
   temp = buf.substr( 0, 6 );
   if( temp != "T_SAMP" )
   {
   ...
   if( temp != "START_TIME_MJL" && temp != "START_TIME_MJD")
   {
  ...
  ofstream mp( "mp.xy" );
  if ( ! mp ) {
    cout << "ERROR: cannot open mp.xy " << endl;
    exit(0);
  }
...
//	      (int) ymdhms.hour << "h" << endl;
        #ifdef GMT4
 	      hourstamps << (xmap+0.02) << " " << ymap << " 6 0 0 ML " <<
	      (int) roundoff(tt) << "h" << endl;
        #endif
        #ifdef GMT5
 	      hourstamps << (xmap+0.02) << " " << ymap << " " <<
	      (int) roundoff(tt) << "h" << endl;
        #endif
  ...
  #ifdef GMT4
	outbat << "psxy " << i << ".sat.xy -R -JX -W0.15p/0 -V -P -M -O -K >> skyplot.ps"
        << endl;
  #endif
  #ifdef GMT5
	outbat << "psxy " << i << ".sat.xy -R -JX -W0.10p,black,solid -V -P -O -K >> skyplot.ps"
        << endl;
  #endif
  ...
  y = 0;
  x = jpi / 2.0 - 0.0;
  //   \260 make the degree symbol
  outring << x << "  " << y  << adj_info << " 0\260" << endl;;

  x = jpi / 2.0 - 30.0 * jpi/ 180.0;
  outring << x << "  " << y  << adj_info << " 30\260" << endl;;


  x = jpi / 2.0 - 60.0 * jpi/ 180.0;
  outring  << x << " " << y << adj_info << " 60\260" << endl;;

  x = jpi / 2.0 - 90.0 * jpi/ 180.0;
  outring  << x << " " << y << adj_info << " 90\260" << endl;;

  x = jpi / 2.0 - cutoffAngle * jpi/ 180.0;
  outring  << -x << " " << y << adj_info << " " <<
    cutoffAngle << "\260" << endl;;
  ...
  nesw << 0 << " " <<  100 * 3.1415/180 << adj_info << " N" << endl;
  nesw << 0 << " " << -100 * 3.1415/180 << adj_info << " S" << endl;
  nesw << -100 * 3.1415/180 << " " << 0 << adj_info << " W" << endl;
  nesw <<  100 * 3.1415/180 << " " << 0 << adj_info << " E" << endl;
  ...
  nesw << x << " " << y  <<  adj_info << " 45\260" << endl;

  x = sin(jpi/4.0)*( rtemp * jpi/180.0) ;
  y = -cos(jpi/4.0)*( rtemp * jpi/180.0) ;
  nesw << x << " " << y  <<  adj_info <<" 135\260" << endl;

  x = -sin(jpi/4.0)*( rtemp * jpi/180.0) ;
  y = -cos(jpi/4.0)*( rtemp * jpi/180.0) ;
  nesw << x << " " << y  <<  adj_info << " 225\260" << endl;

  x = -sin(jpi/4.0)*( rtemp * jpi/180.0) ;
  y = cos(jpi/4.0)*( rtemp * jpi/180.0) ;
  nesw << x << " " << y  <<  adj_info << " 315\260" << endl;

  // create a legend showing the size of the multipath bars
  nesw << " 1.5  -1.59 " << adj_info << " 10 meters" << endl;
  nesw << " 1.5  -1.49 " << adj_info << "  5 meters" << endl;
  nesw << " 1.5  -1.39 " << adj_info << "  1 meter" << endl;

  nesw.close();


  #ifdef GMT4
  outbat << "psxy hr.xy  -R -JX -V  -Sc0.03 -G0/0/0  -O -K -P >> skyplot.ps " << endl;
  outbat << "pstext hr.txt  -R -JX -V -G0/0/255  -O -K -P >> skyplot.ps " << endl;
  outbat << "psxy cross.txt  -R -JX   -V -M -O -K -P >> skyplot.ps " << endl;
  outbat << "pstext nesw.txt  -R  -JX -O -K  -N   >> skyplot.ps " << endl;
  outbat << "psvelo arrows.xy  -R -JX  -L  -W1.0p/255/0/0 -Se1/0.95/12 -A0.0020/0.035/0.025 " <<
    " -N  -H0 -O -K -P -V >>  skyplot.ps " << endl;
  outbat << "pstext ring.txt  -R  -JX -O   -N -W255o  >> skyplot.ps " << endl;
  #endif
  #ifdef GMT5
  outbat << "psxy hr.xy  -R -JX -V  -Sc0.03 -G0/0/0  -O -K -P >> skyplot.ps " << endl;
  outbat << "pstext hr.txt -R -JX -V -F+a0+jML+f6p,Helvetica,blue -O -K -P >> skyplot.ps " << endl;
  outbat << "psxy cross.txt  -R -JX   -V -O -K -P >> skyplot.ps " << endl;
  outbat << "pstext nesw.txt  -R  -JX -F+a0+jCM+f10p,Helvetica,black -O -K  -N   >> skyplot.ps " << endl;
  outbat << "psvelo arrows.xy  -R -JX  -L  -W1.0p,red,solid -Se4/0.95/10 -A0.040/0.045/0.055 " <<
    " -Gred -N  -h0 -O -K -P -V >>  skyplot.ps " << endl;
  outbat << "pstext ring.txt -R -JX -F+a0+jCM+f8p,Helvetica,black -O -N -T -W0.5,black,solid -Gwhite  >> skyplot.ps " << endl;
  #endif
  ...
  outbat << "echo on " << endl;
  outbat << "rm -f *.sat.xy arrows.xy cross.txt cutoffRing.dat elevRings.dat ring.txt hr.txt hr.xy mp.xy nesw.txt skyplot.AzEl title.txt" << endl;
...
int bcread(RinexNavFile &navFile, ofstream &out )
{
  ...
  while( navFile.readPRNBlock( currBlock ) != 0  )
    ...
    } /* end of while loop o.sat.xy -R -JX -W0.10p,black,solid -V -P  -O -K ver all the broadcast blocks */
